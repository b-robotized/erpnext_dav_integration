...
class DAVAccount(Document):
    ...
    def sync_address_books_on_save(self):
        ...
        try:
            ...
            frappe.msgprint(f"Failed to fetch address books: {str(e)}")
...
def create_address_book(docname, address_book_name):
    ...
    create_url = urljoin(
        base_url,
        f"{addressbook_home}/users/{username}/{book_slug}/"
    )

    # Avoid duplicate local entries when name already exists
    existing_book = next(
        (
            x for x in doc.dav_address_books
    ...
    if response.status_code == 201:
        ...
        existing_child = next(
            (
                x for x in doc.dav_address_books
                if x.url and x.url.rstrip("/") == create_url.rstrip("/")
            ),
            None
        )
    ...
    proper_url = create_url.replace(base_url, "/").rstrip("/") if create_url.startswith(base_url) else create_url
    # ✅ Add to child table after creation or existing server dir
    if add_to_child_table:
        doc.append("dav_address_books", {
