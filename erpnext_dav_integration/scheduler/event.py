...
def sync_user_caldav_events(dav_account_name):
    ...
    try:
        ...
        frappe.log_error(f"Check for deleted events {dav_account_name}1")
        ...
        frappe.log_error(
            f"Failed to sync {dav_account_name}: {str(e)}",
            "CalDAV Sync Error"
        )
        raise
    ...
    try:
        ...
        for event_data in events:
            try:
                ...
                frappe.log_error(
                    f"Failed to sync event {event_data.get('uid')}: {str(e)}",
                    "Calendar Event Sync Error"
                )
        ...
        frappe.log_error(
            f"Failed to sync calendar {calendar.get('name')}: {str(e)}",
            "Calendar Sync Error"
        )
    ...
    try:
        ...
        for synced_event in synced_events:
            if synced_event['caldav_event_id'] not in caldav_event_ids:
                ...
                handle_caldav_event_deletion(
                    synced_event['caldav_event_id'],
                    dav_account_name,
                    synced_event
                )
                deleted_count += 1
        ...
        frappe.log_error(
            f"Error checking deleted events: {str(e)}",
...
def handle_caldav_event_deletion(caldav_event_id, dav_account_name, event_record):
    ...
    try:
        ...
        frappe.logger().warning(
            f"Event {event_record['name']} deleted from CalDAV"
        )
        ...
        frappe.log_error(
            f"Failed to handle deletion of {caldav_event_id}: {str(e)}",
            "Deletion Handler Error"
        )
    ...
    try:
        ...
        for event_record in stale_events:
            try:
                ...
                frappe.log_error(
                    f"Failed to cleanup {event_record['name']}: {str(e)}",
                    "Cleanup Error"
                )
...
def log_sync_statistics(dav_account_name):
    ...
    try:
        ...
        total_events = frappe.db.count(
            'Event',
            filters={'caldav_account': dav_account_name}
        )
