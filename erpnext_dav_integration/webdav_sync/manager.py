...
class WebDAVManager:
    ...
    def propfind(self, path: str, depth: str = "1") -> list[dict]:
        ...
        for resp in root.iter():
            ...
            prop = next(
                (e for e in resp.iter() if e.tag.endswith("propstat")), None
            )
            if prop is None:
                continue
            ...
            resourcetype = next(
                (e for e in prop.iter() if e.tag.endswith("resourcetype")), None
            )
    ...
    def get_file(self, path: str) -> bytes:
        ...
        response = self._request(
            "GET", path, headers={"Content-Type": "application/octet-stream"}
        )
    ...
    def put_file(
        ...
        if response.status_code == 412:
            frappe.throw(
                "File was modified by another client.  "
                "Refresh the cloud version and retry."
            )
    ...
    def create_share_link(
        ...
        data = {
            ...
            "expireDate":  expire_date.strftime("%Y-%m-%d") if expire_date else self.file_expire_date.strftime("%Y-%m-%d") if self.dav_account.default_share_link_expire_in else None,
        }
        if password:
    ...
    def _mark_deleted_events(self, current_uids, calendar_url):
        ...
        for event in erp_events:
            if event.caldav_event_id not in current_uids:
                try:
                    WebDAVSyncor.handle_caldav_deletion(
...
class FolderProvisioner:
    ...
    def __init__(self, dav_account_name: str):
        ...
        self.root    = (
            getattr(self.account, "webdav_root_folder", None) or self.DEFAULT_ROOT
        ).strip("/")
        self._files_base = (
            f"/remote.php/dav/files/{self.mgr.username}/{self.root}"
        )
    ...
    def provision_employee_folders(self, employee_doc) -> dict:
        ...
        frappe.logger().info(
            f"FolderProvisioner: Employee {emp_id} → shared={shared}  private={private}"
        )
    ...
    def provision_project_folders(self, project_doc) -> dict:
        ...
        frappe.logger().info(
            f"FolderProvisioner: Project {proj_id} → {base}"
        )
        return {
            "base":      base,
            "invoices":  invoices,
...
class WebDAVSyncor:
    ...
    def sync_folder(self, cloud_path: str, doctype: str, docname: str) -> list[dict]:
        ...
        for resource in resources:
            ...
            try:
                ...
                frappe.log_error(
                    f"WebDAVSyncor: error syncing {resource['href']}: {exc}"
                )
                results.append({
                    "file":     resource["name"],
                    "status":   "error",
    ...
    def _upsert_sync_log(cloud_path, etag, doctype, docname, file_doc_name):
        ...
        if log_name:
            frappe.db.set_value("WebDAV File Sync Log", log_name, {
                "etag":           etag,
    ...
    def pull_invoices_from_folder(self, cloud_folder: str) -> list[dict]:
        ...
        for resource in resources:
            ...
            try:
                ...
                frappe.log_error(
                    f"pull_invoices_from_folder: {resource['href']}: {exc}"
                )
    ...
    def _pull_single_invoice(self, resource: dict) -> dict:
        ...
        # ── skip unchanged ──────────────────────────────────────────────────
        log_name = frappe.db.get_value(
            "WebDAV File Sync Log", {"cloud_path": href}, "name"
        )
    ...
    def pull_contracts_from_folder(self, cloud_folder: str) -> list[dict]:
        ...
        for resource in resources:
            ...
            try:
                ...
                frappe.log_error(
                    f"pull_contracts_from_folder: {resource['href']}: {exc}"
                )
                results.append({"file": resource["name"], "status": "error", "error": str(exc)})
            time.sleep(0.1)
    ...
    def _resolve_or_create_contract_stub(filename: str) -> str:
        ...
        match = frappe.db.get_value(
            ...
            "Contract", {"name": ["like", f"%{stem}%"]}, "name"
        )
        if match:
            return match
    ...
    def sync_all_project_folders(self):
        ...
        for proj in projects:
            ...
            try:
                ...
                frappe.log_error(
                    f"sync_all_project_folders – invoices for {proj['name']}: {exc}"
                )
            try:
                self.pull_contracts_from_folder(f"{base}/Contracts/")
            except Exception as exc:
                frappe.log_error(
                    f"sync_all_project_folders – contracts for {proj['name']}: {exc}"
                )
