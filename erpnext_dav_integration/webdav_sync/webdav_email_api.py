...
def list_folder(path: str, dav_account: str | None = None) -> list[dict]:
    ...
    for r in resources:
        ...
        dav_href = r["href"]        
        relative_path = dav_href.split(
            f"/remote.php/dav/files/{mgr.username}",
            1
        )[-1]

        preview_url = (
            f"{base_url}/core/preview"
...
def create_share_link(
    ...
    share = mgr.create_share_link(cloud_path, permissions=1, password=password or None, expire_date=frappe.utils.get_datetime(expire_date) if expire_date else None)
