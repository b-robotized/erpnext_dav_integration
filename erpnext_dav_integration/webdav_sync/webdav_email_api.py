...
def _resolve_dav_account(dav_account: str | None = None) -> str:
	...
	if frappe.db.exists("DAV Account", {"username": user, "enabled": 1}) or frappe.db.exists(
		"DAV Account", {"user": user, "enabled": 1}
	):
		return frappe.db.get_value(
			"DAV Account", {"username": user, "enabled": 1}, "name"
		) or frappe.db.get_value("DAV Account", {"user": user, "enabled": 1}, "name")
	else:
		frappe.throw(
			_(
