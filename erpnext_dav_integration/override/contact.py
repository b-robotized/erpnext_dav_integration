...
def create_contact(dav, doc, vcard, password):
    base = doc.custom_dav_address_book_url.rstrip("/") if doc.custom_dav_address_book_url.startswith("http") else dav.base_url.rstrip("/") + doc.custom_dav_address_book_url.rstrip("/")
...
def build_vcard(doc):
    ...
    for social in doc.get("custom_social_profiles", []):
        vcard.append(
            "X-SOCIALPROFILE;TYPE={}:{}".format(
                social.get("type", "OTHER"),
                nn(social.get("url"))
            )
        )
...
def delete_contact_from_carddav(doc, method):
    try:
        ...
        res = requests.delete(
            doc.custom_vcard_url,
            auth=HTTPBasicAuth(dav.username, password)
        )

        if res.status_code not in (200, 204):
            frappe.log_error(f"Failed to delete contact from CardDAV: {res.status_code} - {res.text}", "DAV Sync Delete Error")
