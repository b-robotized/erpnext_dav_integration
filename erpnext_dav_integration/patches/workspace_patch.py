...
def execute():
	...
	exists = any(
		b.get("type") == "card" and b.get("data", {}).get("card_name") == "Nextcloud Services" for b in blocks
	)
	...
	if ("Card Break", "Nextcloud Services") not in existing_links:
		ws.append("links", {"type": "Card Break", "label": "Nextcloud Services"})

	for label in ["DAV Account"]:
		if ("Link", label) not in existing_links:
			ws.append("links", {"type": "Link", "label": label, "link_type": "DocType", "link_to": label})
	...
	# 3. SIDEBAR (🔥 NEW PART)
	# ========================
	sidebar = frappe.get_all("Workspace Sidebar", filters={"name": workspace_name}, limit=1)

	if sidebar:
		sidebar_doc = frappe.get_doc("Workspace Sidebar", sidebar[0].name)
	else:
		sidebar_doc = frappe.get_doc({"Workspace Sidebar", "integrations"})
	...
	if ("Section Break", "Nextcloud Services") not in existing_sidebar:
		sidebar_doc.append(
			"items", {"type": "Section Break", "label": "Nextcloud Services", "icon": "integration"}
		)
	...
	for label in ["DAV Account"]:
		if ("Link", label) not in existing_sidebar:
			sidebar_doc.append(
				"items",
				{"type": "Link", "label": label, "link_type": "DocType", "link_to": label, "child": 1},
			)
	...
	# SAVE PAGE
	# ========================
	save_page(name=workspace_name, public=1, new_widgets=json.dumps([]), blocks=json.dumps(blocks))
