            ...
            () => {
                const browser = new erpnext_dav_integration.webdav.FileBrowser({
                    ...
                    on_link(url, filename) {
                        ...
                        const $content = frm.fields_dict.content
                            && frm.fields_dict.content.$input_wrapper
                        ...
                        if ($content && $content.length) {
                            ...
                            if (quill) {
                                ...
                                quill.insertEmbed(index + 1, "link", { href: url, innerText: filename });
                            } else {
                                $content.append(
                                    `<p><a href="${frappe.utils.escape_html(url)}">${frappe.utils.escape_html(filename)}</a></p>`
                                );
                            }
                        } else {
                            // Fallback: copy to clipboard
                            frappe.utils.copy_to_clipboard(url);
                            frappe.show_alert({ message: __("Share link copied to clipboard"), indicator: "green" });
                        }
                    },
                });
                browser.show();
...
frappe.views.CommunicationComposer.prototype._inject_cloud_storage_button = function () {
    ...
    const $attach_btn = composer.dialog.$wrapper.find(
...
frappe.after_ajax(() => {
    ...
    const try_hook_composer = () => {
        ...
        frappe.views.CommunicationComposer.prototype._inject_cloud_storage_button = function () {
            ...
            const $attach_area = composer.dialog.$wrapper.find(".attachment-control, .btn-attach-files").first();
            ...
            $cloud_btn.on("click", () => {
                ...
                const docname = composer.doc && composer.doc.name
                    ? composer.doc.name
                    : "new-communication-1";
