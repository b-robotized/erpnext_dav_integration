...
erpnext_dav_integration.webdav.FileBrowser = class WebDAVFileBrowser {
	...
	_skeleton_html() {
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;
          ...
          <div class="wdav-sel-bar"
            ...
            <span style="font-size:13px;color:var(--color-text-tertiary)">${__(
				"Select a file above"
			)}</span>
          </div>
          ...
          <div style="display:flex;justify-content:flex-end;gap:8px;padding-top:12px">
            <button class="btn btn-default btn-sm wdav-btn-link" disabled
                    title="${__(
						"Create a Nextcloud share link and insert it into the email body"
					)}">
	...
	_render_breadcrumb() {
		const parts = this._path_stack.map((item, i) => {
			...
			if (is_last) {
				return `<span style="font-size:12px;font-weight:500;color:var(--color-text-primary)">${frappe.utils.escape_html(
					item.label
				)}</span>`;
			}
			return `
                <span class="wdav-bc-item" data-idx="${i}"
                    ...
                    ${
						i === 0
							? '<i class="ti ti-home" style="font-size:12px" aria-hidden="true">All Files</i>'
	...
	_render_items(items) {
		...
		const rows = items.map((item) => {
			...
			const meta = is_folder
				? `<span style="font-size:11px;color:var(--color-text-tertiary)">${__(
						"folder"
				  )}</span>`
				: `<span style="font-size:11px;color:var(--color-text-tertiary)">${frappe.utils.escape_html(
						item.size_display
				  )}</span>`;
            ...
            <div class="wdav-item"
                ...
                <span style="width:22px;text-align:center;flex-shrink:0">${_icon_html(
					item.icon_type
				)}</span>
		...
		this.$file_list.find(".wdav-item").on({
			click: (e) => {
				...
				if (is_folder) {
					...
					this._select_item($row, { href, name, id }, e.ctrlKey || e.metaKey);
				}
			},
			dblclick: (e) => {
				const $row = $(e.currentTarget);
				...
				if (is_folder) {
					this._navigate_to($row.data("href"), $row.data("name"));
				}
			},
		});
	}
				...
				callback: (r) => {
					if (r.message && r.message.url) {
						...
						this._set_error_state(
							__("Could not create share link for {0}.", [item.name])
						);
					}
				},
	...
	_set_loading_state(message) {
		this.$sel_bar.html(`
            ...
            <span style="font-size:13px;color:var(--color-text-secondary)">${frappe.utils.escape_html(
				message
			)}</span>`);
		this.$btn_attach.prop("disabled", true);
		this.$btn_link.prop("disabled", true);
	...
	_set_success_state(message) {
		this.$sel_bar.html(`
            ...
            <span style="font-size:13px;color:var(--color-text-success)">${frappe.utils.escape_html(
				message
			)}</span>`);
	}
	...
	_set_error_state(message) {
		this.$sel_bar.html(`
            ...
            <span style="font-size:13px;color:var(--color-text-danger)">${frappe.utils.escape_html(
				message
			)}</span>`);
		this.$btn_attach.prop("disabled", false);
		this.$btn_link.prop("disabled", false);
	...
	_get_preview_html(item) {
		const is_image = ["jpg", "jpeg", "png", "gif", "webp", "bmp", "svg"].includes(
			(item.name || "").split(".").pop()?.toLowerCase()
		);
		console.log(
			"Checking if item is image for preview:",
			item.name,
			"→",
			is_image,
			"preview_url:",
