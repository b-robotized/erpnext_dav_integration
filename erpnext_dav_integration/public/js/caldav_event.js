...
frappe.ui.form.on("Event", {
		...
		if (frm.doc.caldav_account) {
			...
			if (user && user !== "Guest") {
				if (!frm.doc.caldav_organizer) {
					frm.set_value("caldav_organizer", user !== "Administrator" ? user : null);
				}
				...
				callback: function (r) {
					if (r.message) {
						if (r.message.length > 0) {
							...
							frappe.msgprint(
								__(
									"Selected calendar not found on server. Please sync your calendars and try again."
								)
							);
	...
	onload: function (frm) {
		if (
			...
			frm.doc.caldav_organizer != frappe.session.user
		) {
